...
describe('sqlForPartialUpdate', () => {
  test('works: valid data w/ complete jsToSql', () => {
    const data = { firstName: 'Leshawn', lastName: 'Rice', age: 22, numEmployees: 3, logoUrl: 'google.com' };
    ...
    expect(values).toEqual(['Leshawn', 'Rice', 22, 3, 'google.com']);
  ...
  test('works: valid data w/ incomplete jsToSql', () => {
    const data = { first_name: 'Leshawn', lastName: 'Rice', age: 22, numEmployees: 3, logoUrl: 'google.com' };
...
describe('sqlFilterSelect', () => {
  test('works: valid complete data w/ complete jsToSql', () => {
    const data = { name: 'Leshawn', minEmployees: 3, maxEmployees: 10 };
    ...
    expect(values).toEqual(['Leshawn', 3, 10]);
  ...
  test('works: valid just-name data w/ just-name jsToSql', () => {
    const data = { name: 'Leshawn' };
    const jsToSql = { name: 'name' }

    const { setCols, values } = sqlFilterSelect(data, jsToSql);

    expect(values).toEqual(['Leshawn']);
